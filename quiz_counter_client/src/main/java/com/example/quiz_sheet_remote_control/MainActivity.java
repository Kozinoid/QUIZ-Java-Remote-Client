...
import com.example.quiz_counter_client.R;
...
public class MainActivity extends AppCompatActivity {
    ...
    Handler.Callback onServerFound = new Handler.Callback() {
        ...
        public boolean handleMessage(@NonNull Message msg) {
            ...
            {
                tcpConnectionIDText = GetConnectionID("/" + msg.obj, String.valueOf(tcpPort));
                //Log.d("myLogs", tcpConnectionIDText);
                myTCPDevice.Connect(tcpConnectionIDText, onRecieveMessage);
            }
            return false;
        }
    ...
    private void SendOneTeam(String name, int score)
    {
        ...
        myTCPDevice.onSendRequest(message);
    }
    ...
    {
        sPref = getPreferences(MODE_PRIVATE);
        tcpConnectionIDText = sPref.getString(SAVED_TEXT, "tcp://192.168.0.99:8060/");
        ...
        public void onClick(View v) {
            switch (v.getId())
            {
                ...
                case R.id.bt_connect:
                    tcpConnectionIDText = GetConnectionID(mQuickNameEditText.getText().toString(), String.valueOf(tcpPort));
                    mQuickNameEditText.setText("");
                    mIPTextView.setText(tcpConnectionIDText);
                    try {
                        Connect();
